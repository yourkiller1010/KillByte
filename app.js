// ==UserScript==
// @name         SL TECH BD V2.6 – Free (No License)
// @version      2.6-free
// @description  All features unlocked, no license required. Leaderboard, deposit, transactions, etc.
// @author       SL (modified)
...
(function () {
  ...
  function getLiveBalance(){
    ...
    __cachedBalance = numFromText(document.querySelector(selectors.usermenuBalance)?.textContent) || 0;
  ...
  function updateUI() {
    ...
    if (!isNaN(bal) && profitEl) {
      ...
      if (diff < 0) {
        ...
        profitEl.classList.add('---react-features-Sidepanel-LeaderBoard-Position-styles-module__red--qUPWg');
      } else {
        profitEl.innerText = `$${formatWithThousands(diff)}`;
        profitEl.classList.remove('---react-features-Sidepanel-LeaderBoard-Position-styles-module__red--qUPWg');
    ...
    // sync level icon to popup
    try {
      const popupIconUse = document.querySelector(".panel-leader-board__information-name svg use");
      if (popupIconUse) popupIconUse.setAttribute("xlink:href", iconHref);
    } catch(e){}

    // apply avatar from localStorage
    try {
      let _lb = JSON.parse(localStorage.getItem("lbData") || "{}");
      if (_lb.avatar) {
        ...
        if (rowAvatar) rowAvatar.innerHTML = `<img src="${_lb.avatar}" style="width:16px;height:16px;border-radius:50%">`;
  ...
  function formatAccountDropdownBalances() {
    document.querySelectorAll('b.---react-features-Usermenu-Dropdown-styles-module__selectBalance--IfQIW').forEach(el => {
  ...
  function getYourData() {
    ...
    const moneyEl = document.querySelector('.---react-features-Sidepanel-LeaderBoard-Position-styles-module__money--BwWCZ');
  ...
  function updateFooter(positionNum) {
    ...
    if (!footer) return;
    footer.innerHTML = `<span style="color: var(--color-black-50); font-weight: 500;">Your position:&nbsp;&nbsp;</span><span style="font-weight: 700;">${positionNum}</span>`;
  }
  ...
  function calculateInterpolatedPosition(profit){
    ...
    if (profit < 1000) return Math.max(2000 - Math.floor(profit * 1.5), 100);
    if (profit < 5000) return Math.max(500 - Math.floor((profit - 1000) / 10), 20);
    if (profit < 10000) return Math.max(20 - Math.floor((profit - 5000) / 500), 2);
    return 1;
  }
  ...
  function openDepositPopup() {
    ...
    popup.innerHTML = `
      ...
      <div style="position: relative; width: 100%; margin-bottom: 6px;">
        ...
        <div id="sliderPercentDisplay" style="position: absolute; top: -22px; right: 10px; font-weight: bold; color: #0077cc; background: rgba(0,0,0,0.1); padding: 2px 6px; border-radius: 4px; user-select: none; pointer-events: none;">0%</div>
      </div>
    ...
    style.textContent = `
      .sl-input { width:100%; padding:12px; margin-bottom:10px; border:1px solid #ccc; border-radius:10px; background:#f9f9f9; color:#333; font-size:15px; outline:none; }
      .sl-input:focus { border-color:#0077cc; box-shadow:0 0 5px rgba(0,119,204,0.4); }
      .sl-button { padding:10px 20px; margin:0 6px; background:#0077cc; border:none; border-radius:8px; color:#fff; font-weight:bold; cursor:pointer; }
    ...
    const expandSpan = document.querySelector('.---react-features-Sidepanel-LeaderBoard-Position-styles-module__loading--h38TV .---react-features-Sidepanel-LeaderBoard-Position-styles-module__expand--KBHoM');
    slider.oninput = () => {
    ...
    $('#setCapitalBtn').onclick = () => {
      ...
      if (headerNameDiv && name && flag) {
        headerNameDiv.innerHTML = `<svg class="flag-${flag}"><use xlink:href="/profile/images/flags.svg#flag-${flag}"></use></svg> ${name}`;
      }
      const ok = document.createElement("div");
      ok.innerHTML = "✅ <b>Verified & Applied Successfully!</b><br><span style='font-size:13px;'>Name updated on leaderboard</span>";
      Object.assign(ok.style, { position:"fixed", top:"20px", right:"20px", background:"#0faf59", color:"#fff", padding:"14px 18px", borderRadius:"12px", boxShadow:"0 10px 25px rgba(0,0,0,0.25)", zIndex:999999 });
  ...
  document.addEventListener("keydown", e => {
    if (e.ctrlKey && e.shiftKey && e.key.toLowerCase() === "s") openDepositPopup();
  });
    ...
    (function helpButtonFullscreenLock(){
      function enableFullscreenAndBlockRefresh(){
        ...
        window.addEventListener("keydown", function(e){
          ...
          if (k === "f5" || (e.ctrlKey && k === "r") || (e.ctrlKey && e.shiftKey && k === "r")) {
            e.preventDefault();
            e.stopPropagation();
          }
        }, true);
      ...
      bindHelpBtn();
      new MutationObserver(bindHelpBtn).observe(document.body, { childList:true, subtree:true });
    })();
  },800);

  // start leaderboard monitor after load
  window.addEventListener('load', () => setTimeout(startLeaderboardMonitor, 1000));

})();

/* ===== ADDITIONAL PATCHES (AVATAR, FLAG, POPUP OVERRIDE) ===== */
...
(function(){
  ...
  function isMyName(name){ const d = getData(); return d.name && name === d.name; }
  function setRowAvatar(row, url){
    const box = row.querySelector(".---react-features-Sidepanel-LeaderBoard-styles-module__block--zCluU");
    if(!box) return;
    box.innerHTML = `<img src="${url}" style="width:18px;height:18px;border-radius:50%;object-fit:cover">`;
  }
  function setPopupAvatar(url){
    const wrap = document.querySelector(".panel-leader-board__information-avatar");
    if(!wrap) return;
    wrap.innerHTML = `<img src="${url}" style="width:42px;height:42px;border-radius:50%;object-fit:cover">`;
  }
  setInterval(()=>{
    const d = getData();
    ...
    document.querySelectorAll(".---react-features-Sidepanel-LeaderBoard-styles-module__item--8FRDh").forEach(row=>{
      ...
      if(!nameEl) return;
      if(isMyName(nameEl.textContent.trim())) setRowAvatar(row, d.avatar);
    });
    const popupName = document.querySelector(".panel-leader-board__information-name")?.textContent?.trim();
    if(isMyName(popupName)) setPopupAvatar(d.avatar);
    else {
      const wrap = document.querySelector(".panel-leader-board__information-avatar");
      if(wrap && wrap.querySelector("img")) wrap.innerHTML = '';
    }
  }, 500);
})();
...
(function(){
  try{
    ...
    function apply(row){
      ...
      if(!myName) return;
      if(nameEl.textContent.trim() !== myName) return;
      const def = block.querySelector("svg.icon-avatar-default");
      if(def) def.remove();
      const emptyDiv = block.querySelector(".---react-features-Sidepanel-LeaderBoard-styles-module__avatar--ZVpcN");
    ...
    function run(){ document.querySelectorAll('[class*="LeaderBoard-styles-module__item"]').forEach(apply); }
    run();
...
(function(){
  function moneyFmt(num){ return Number(num).toLocaleString("en-US", { minimumFractionDigits: 2, maximumFractionDigits: 2 }); }
  const KEY = atob("c2x0ZWNoX2xlYWRlcmJvYXJkX2RhdGE=");
  const countryMap = { bd:"Bangladesh", in:"India", pk:"Pakistan", ae:"United Arab Emirates", tr:"Turkey", lk:"Sri Lanka", br:"Brazil" };
  function getMyData(){ try { return JSON.parse(localStorage.getItem(KEY) || "{}"); } catch(e){ return {}; } }
  const __statCache = {};
  function realisticStats(profit){
    const key = String(profit);
    if (__statCache[key]) return __statCache[key];
    const trades = Math.max(9, Math.min(154, Math.floor(20 + Math.abs(profit) % 135)));
    const profitable = Math.max(1, Math.floor(trades * 0.58));
    const minTrade = Math.max(1, Math.floor(profit / 25));
    const maxTrade = Math.min(3000, Math.floor(profit / 2.5));
    __statCache[key] = { trades, profitable, avg: moneyFmt(profit / profitable), min: moneyFmt(minTrade), max: moneyFmt(maxTrade) };
    return __statCache[key];
  }
  function syncLevelIconToPopup(popup){
    try {
      const userLevelUse = document.querySelector('.---react-features-Usermenu-Dropdown-styles-module__levelIcon--lmj_k use');
      const popupLevelUse = popup.querySelector('.---react-features-Sidepanel-LeaderBoard-Information-styles-module__status--xjubR use');
      if(userLevelUse && popupLevelUse) popupLevelUse.setAttribute("xlink:href", userLevelUse.getAttribute("xlink:href"));
    } catch(e){}
  }
  function overridePopup(row){
    const my = getMyData();
    if(!my.name) return;
    const rowName = row.querySelector(".---react-features-Sidepanel-LeaderBoard-styles-module__name--MrPOZ")?.textContent?.trim();
    if(rowName !== my.name) return;
    const popup = document.querySelector(".---react-features-Sidepanel-LeaderBoard-Information-styles-module__information--QuHyA");
    if(!popup) return;
    const profitText = row.querySelector(".---react-features-Sidepanel-LeaderBoard-styles-module__money--jJUGd")?.textContent || "$0";
    ...
    const countryEl = popup.querySelector('[class*="__location"]');
    if(countryEl) countryEl.textContent = countryMap[my.flag] || my.flag.toUpperCase();
    ...
    if(values.length >= 6){
      ...
      values[4].textContent = `$${moneyFmt(parseFloat(stats.min.replace(/,/g, "")))}`;
      values[5].textContent = `$${moneyFmt(parseFloat(stats.max.replace(/,/g, "")))}`;
    }
...
/* ===== TRANSACTION ADDER (STEALTHY) ===== */
(function (){
  'use strict';
  const PROCESS_MS = 120000;
  const payments = [
  ...
  function createRowHTML(d){
    ...
    if(d.status==='Successed'){
      ...
      statusHTML = `<div class="---react-ui-TransactionsScreenItem-styles-module__status-icon--cnv6i ---react-ui-TransactionsScreenItem-styles-module__muted--FGTfS"><svg class="icon-pending"><use xlink:href="/profile/images/spritemap.svg#icon-pending"></use></svg></div><span class="---react-ui-TransactionsScreenItem-styles-module__status-text--RB6BR undefined">Processing</span>`;
    } else {
    ...
    return `<div class="---react-ui-TransactionsScreenItem-styles-module__transaction--iJpIP custom-tx tx-${d.id}" data-datetime="${d.dateObj.getTime()}">
      ...
      <div class="---react-ui-TransactionsScreenItem-styles-module__amount-col--B8L9k">
        <b class="---react-ui-TransactionsScreenItem-styles-module__amount--h5o4H ${d.colorClass}">${d.amountStr}</b>
      </div>`;
  }
  ...
  attachProfilePopup();
  new MutationObserver(attachProfilePopup).observe(document.body,{childList:true,subtree:true});
...
(function(){
  ...
  new MutationObserver(saveRows).observe(document.body,{childList:true,subtree:true});
...
setInterval(()=>{
  document.querySelectorAll(".---react-features-Sidepanel-LeaderBoard-styles-module__item--8FRDh svg.flag").forEach(f=>f.remove());
}, 500);
